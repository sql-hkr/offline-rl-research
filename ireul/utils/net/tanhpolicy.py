...
from ireul.utils.net.common import BasePolicy
from ireul.utils.net.continuous import ActorProb
...
class TanhNormal(Distribution):
    ...
    def sample_n(self, n, return_pre_tanh_value=False):
        ...
        if return_pre_tanh_value:
            return (self.max_action - self.min_action) / 2 * torch.tanh(z) + (
    ...
    def log_prob(self, value, pre_tanh_value=None):
        ...
        :return:
        """
        unscaled_value = (2 * value - (self.max_action + self.min_action)) / (
            self.max_action - self.min_action
        )  # assume the actual actions have been transformed
        if pre_tanh_value is None:
            pre_tanh_value = self.atanh(
                unscaled_value
            )  # get the raw Gaussian distribution output
        ...
        log_prob = self.normal.log_prob(pre_tanh_value) - torch.log(
            action_scale * (1 - squashed_action.pow(2)) + self.epsilon
        )
        return log_prob
