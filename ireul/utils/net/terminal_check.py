...
def termination_fn_halfcheetah(obs, act, next_obs):
    ...
    not_done = np.logical_and(
        np.all(next_obs > -100, axis=-1), np.all(next_obs < 100, axis=-1)
    )
    done = ~not_done
    done = done[:, None]
    return done
...
def termination_fn_other(obs, act, next_obs, global_min_obs, global_max_obs):
    ...
    not_done = np.logical_and(
        np.all(next_obs > global_min_obs - obs_range, axis=-1),
        np.all(next_obs < global_max_obs + obs_range, axis=-1),
    )
    done = ~not_done
    done = done[:, None]
    return done
...
def is_terminal(obs, act, next_obs, task, global_min_obs, global_max_obs):
    if "halfcheetahvel" in task:
        ...
        return termination_fn_other(
            obs, act, next_obs, global_min_obs, global_max_obs
        )
