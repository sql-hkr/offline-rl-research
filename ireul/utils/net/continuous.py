...
from ireul.utils.net.common import MLP, BasePolicy
...
class GaussianActor(nn.Module, BasePolicy):
    def __init__(self, obs_dim, action_dim, hidden_size, hidden_layers):
        ...
        self.register_parameter(
            "max_logstd",
            torch.nn.Parameter(torch.ones(action_dim) * 1, requires_grad=True),
        )
        self.register_parameter(
            "min_logstd",
            torch.nn.Parameter(
                torch.ones(action_dim) * -10, requires_grad=True
            ),
        )
...
class DistributionalCritic(torch.nn.Module):
    def __init__(
        ...
        if self.min_value is not None and self.max_value is not None:
            self.register_buffer(
                "z", torch.linspace(min_value, max_value, atoms)
            )
    ...
    def set_interval(self, min_value, max_value):
        ...
        self.register_buffer(
            "z", torch.linspace(min_value, max_value, self.atoms).to(param)
        )
    ...
    def get_target(self, obs, action, reward, discount):
        ...
        target_z = torch.clamp(
            target_z, self.min_value, self.max_value
        )  # [*B, N]
