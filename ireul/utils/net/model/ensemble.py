...
from ireul.utils.function import soft_clamp
from ireul.utils.net.common import Swish
...
class EnsembleLinear(torch.nn.Module):
    def __init__(self, in_features, out_features, ensemble_size=7):
        ...
        self.register_parameter(
            "weight",
            torch.nn.Parameter(
                torch.zeros(ensemble_size, in_features, out_features)
            ),
        )
        self.register_parameter(
            "bias",
            torch.nn.Parameter(torch.zeros(ensemble_size, 1, out_features)),
        )

        torch.nn.init.trunc_normal_(
            self.weight, std=1 / (2 * in_features**0.5)
        )

        self.register_parameter(
            "saved_weight", torch.nn.Parameter(self.weight.detach().clone())
        )
        self.register_parameter(
            "saved_bias", torch.nn.Parameter(self.bias.detach().clone())
        )
    ...
    def set_select(self, indexes):
        assert (
            len(indexes) <= self.ensemble_size
            and max(indexes) < self.ensemble_size
        )
        self.select = indexes
...
class EnsembleTransition(torch.nn.Module):
    def __init__(
        ...
        for i in range(hidden_layers):
            if i == 0:
                module_list.append(
                    EnsembleLinear(
                        obs_dim + action_dim, hidden_features, ensemble_size
                    )
                )
            else:
                module_list.append(
                    EnsembleLinear(
                        hidden_features, hidden_features, ensemble_size
                    )
                )
        self.backbones = torch.nn.ModuleList(module_list)

        self.output_layer = EnsembleLinear(
            hidden_features, 2 * (obs_dim + self.with_reward), ensemble_size
        )
        self.obs_mean = None
        self.obs_std = None
        self.register_parameter(
            "max_logstd",
            torch.nn.Parameter(
                torch.ones(obs_dim + self.with_reward) * 1, requires_grad=True
            ),
        )
        self.register_parameter(
            "min_logstd",
            torch.nn.Parameter(
                torch.ones(obs_dim + self.with_reward) * -5, requires_grad=True
            ),
        )
    ...
    def forward(self, obs_action):
        ...
        if self.obs_mean is not None:
            if dims:
                obs_mean = (
                    self.obs_mean.unsqueeze(0)
                    .expand(obs_action.shape[0], -1)
                    .to(obs_action.device)
                )
                obs_std = (
                    self.obs_std.unsqueeze(0)
                    .expand(obs_action.shape[0], -1)
                    .to(obs_action.device)
                )
            else:
            ...
            if self.mode == "normalize":
                ...
                obs, action = torch.split(
                    obs_action,
                    [self.obs_dim, obs_action.shape[-1] - self.obs_dim],
                    dim=-1,
                )
                if dims:
                    obs = obs - obs_mean.unsqueeze(dims).expand(
                        -1, batch_size, -1
                    )
                    obs = obs / (
                        obs_std.unsqueeze(dims).expand(-1, batch_size, -1)
                        + 1e-8
                    )
                else:
                    obs = obs - obs_mean.unsqueeze(dims).expand(batch_size, -1)
                    obs = obs / (
                        obs_std.unsqueeze(dims).expand(batch_size, -1) + 1e-8
                    )
