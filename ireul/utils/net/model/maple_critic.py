...
class Maple_critic(nn.Module):
    def __init__(
        ...
        self.mlp = miniblock(
            self.lstm_hidden_unit, self.hidden_sizes[0], None, relu=False
        )
        if len(self.hidden_sizes) >= 2:
            for i in range(1, len(self.hidden_sizes)):
                self.mlp += miniblock(
                    self.hidden_sizes[i - 1], self.hidden_sizes[i], None
                )
        self.mlp = nn.Sequential(*self.mlp)
        self.vfs = miniblock(
            self.hidden_sizes[-1] + self.obs_dim + self.action_dim,
            self.value_hidden_sizes[0],
            None,
        )
        if len(self.value_hidden_sizes) >= 2:
            for i in range(1, len(self.value_hidden_sizes)):
                self.vfs += miniblock(
                    self.value_hidden_sizes[i - 1],
                    self.value_hidden_sizes[i],
                    None,
                )
