...
from ireul.utils.net.common import miniblock
...
class Maple_actor(nn.Module):
    def __init__(
        ...
        self.mlp = miniblock(
            lstm_hidden_unit, hidden_sizes[0], None, relu=False
        )
        if len(hidden_sizes) >= 2:
            for i in range(1, len(hidden_sizes)):
                self.mlp += miniblock(
                    hidden_sizes[i - 1], hidden_sizes[i], None
                )
        self.mlp = nn.Sequential(*self.mlp)
        self.Guassain_input_dim = self.hidden_sizes[-1] + self.obs_dim
        self.Guassain_mlp = miniblock(
            self.Guassain_input_dim, self.Guassain_hidden_sizes[0], None
        )
        if len(Guassain_hidden_sizes) >= 2:
            for i in range(1, len(Guassain_hidden_sizes)):
                self.Guassain_mlp += miniblock(
                    Guassain_hidden_sizes[i - 1],
                    Guassain_hidden_sizes[i],
                    None,
                )
        self.Guassain_mlp = nn.Sequential(*self.Guassain_mlp)
        self.Guassain_mu_mlp = [
            nn.Linear(self.Guassain_hidden_sizes[-1], action_dim)
        ]
        self.Guassain_logstd_mlp = [
            nn.Linear(self.Guassain_hidden_sizes[-1], action_dim)
        ]
    ...
    def gaussian_likelihood(self, x, mu, log_std):
        pre_sum = -0.5 * (
            ((x - mu) / (torch.exp(log_std) + self.EPS)) ** 2
            + 2 * log_std
            + np.log(2 * np.pi)
        )
    ...
    def apply_squashing_func(self, mu, pi, logp_pi):
        logp_pi -= torch.sum(
            2 * (np.log(2) - pi - F.softplus(-2 * pi)), dim=-1
        )
