...
from ireul.utils.function import asymmetric_l2_loss
...
def algo_init(args):
    ...
    iql_v = Critic(
        preprocess_net=net_iql_v,
        hidden_layer_size=args["hidden_layer_size"],
    ).to(args["device"])
    iql_v_optim = optim.Adam(iql_v.parameters(), lr=args["critic_lr"])
    ...
    iql_critic1 = Critic(
        preprocess_net=iql_net_c1,
        hidden_layer_size=args["hidden_layer_size"],
    ).to(args["device"])
    iql_critic1_optim = optim.Adam(
        iql_critic1.parameters(), lr=args["critic_lr"]
    )
    ...
    iql_critic2 = Critic(
        preprocess_net=iql_net_c2,
        hidden_layer_size=args["hidden_layer_size"],
    ).to(args["device"])
    iql_critic2_optim = optim.Adam(
        iql_critic2.parameters(), lr=args["critic_lr"]
    )
    ...
    nets = {
        ...
        # IQL
        "iql_v": {"net": iql_v, "opt": iql_v_optim},
        "iql_critic1": {"net": iql_critic1, "opt": iql_critic1_optim},
        "iql_critic2": {"net": iql_critic2, "opt": iql_critic2_optim},
    }
...
class AlgoTrainer(BaseAlgo):
    def __init__(self, algo_init, args):
        ...
        # IQL
        self.iql_v = algo_init["iql_v"]["net"]
        self.iql_v_opt = algo_init["iql_v"]["opt"]
        self.iql_critic1 = algo_init["iql_critic1"]["net"]
        self.iql_critic1_opt = algo_init["iql_critic1"]["opt"]
        self.iql_critic2 = algo_init["iql_critic2"]["net"]
        self.iql_critic2_opt = algo_init["iql_critic2"]["opt"]
        self.iql_critic1_target = copy.deepcopy(self.iql_critic1)
        self.iql_critic2_target = copy.deepcopy(self.iql_critic2)
    ...
    def _train(self, batch):
        ...
        """
        with torch.no_grad():
            iql_next_v = self.iql_v(next_obs)
            iql_target_q = torch.min(
                self.iql_critic1_target(obs, actions),
                self.iql_critic2_target(obs, actions),
            )

        iql_v = self.iql_v(obs)
        iql_adv = iql_target_q - iql_v
        iql_v_loss = asymmetric_l2_loss(iql_adv, self.args["iql_tau"])
        self.iql_v_opt.zero_grad()
        iql_v_loss.backward()
        self.iql_v_opt.step()
        ...
        iql_q1_pred = self.iql_critic1(obs, actions)
        iql_q2_pred = self.iql_critic2(obs, actions)
        iql_qf1_loss = F.mse_loss(iql_q1_pred, iql_targets)
        iql_qf2_loss = F.mse_loss(iql_q2_pred, iql_targets)
        self.iql_critic1_opt.zero_grad()
        iql_qf1_loss.backward(retain_graph=True)
        self.iql_critic1_opt.step()
        self.iql_critic2_opt.zero_grad()
        iql_qf2_loss.backward()
        self.iql_critic2_opt.step()
        ...
        # IQL
        qf1_ood_loss = F.mse_loss(
            self.critic1(obs, _a), self.iql_critic1(obs, _a).detach()
        )
        qf2_ood_loss = F.mse_loss(
            self.critic2(obs, _a), self.iql_critic2(obs, _a).detach()
        )
        ...
        # IQL
        self._sync_weight(
            self.iql_critic1_target,
            self.iql_critic1,
            self.args["soft_target_tau"],
        )
        self._sync_weight(
            self.iql_critic2_target,
            self.iql_critic2,
            self.args["soft_target_tau"],
        )
