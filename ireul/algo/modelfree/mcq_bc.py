...
def algo_init(args):
    ...
    bc = TanhGaussianPolicy(
        preprocess_net=net_bc,
        action_shape=action_shape,
        hidden_layer_size=args["hidden_layer_size"],
        conditioned_sigma=True,
    ).to(args["device"])

    bc_optim = optim.Adam(bc.parameters(), lr=args["actor_lr"])
...
class AlgoTrainer(BaseAlgo):
    def __init__(self, algo_init, args):
        ...
        self.bc = algo_init["bc"]["net"]
        self.bc_optim = algo_init["bc"]["opt"]
    ...
    def _get_tensor_values(self, obs, bc=None, network=None):
        if bc is None:
    ...
    def _train(self, batch):
        ...
        _, pre_tanh_value = tanh_normal.rsample(return_pretanh_value=True)
        bc_loss = -torch.mean(
            tanh_normal.log_prob(actions, pre_tanh_value=pre_tanh_value)
        )
        ...
        pesudo_q1_curr_target, q1_curr_act = self._get_tensor_values(
            obs, network=self.critic1, bc=self.bc
        )
        pesudo_q1_next_target, q1_next_act = self._get_tensor_values(
            next_obs, network=self.critic1, bc=self.bc
        )
        ...
        pesudo_q2_curr_target, q2_curr_act = self._get_tensor_values(
            obs, network=self.critic2, bc=self.bc
        )
        pesudo_q2_next_target, q2_next_act = self._get_tensor_values(
            next_obs, network=self.critic2, bc=self.bc
        )
        ...
        )  # + torch.std(q1_ood_pred - (pesudo_q1_target + pesudo_q2_target) / 2)
        qf2_loss = (
            self.lam * self.critic_criterion(q2_pred, q_target)
            + (1 - self.lam) * qf2_ood_loss
        )  # + torch.std(q2_ood_pred - (pesudo_q1_target + pesudo_q2_target) / 2)
