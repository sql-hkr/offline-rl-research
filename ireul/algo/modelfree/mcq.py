...
class AlgoTrainer(BaseAlgo):
    ...
    def _get_tensor_values(self, obs, vae=None, network=None):
        ...
        if vae is None:
            repeat_actions, _ = self.forward(
                obs_repeat, reparameterize=True, return_log_prob=True
            )
            ...
            repeat_actions = repeat_actions.reshape(
                self.num * self.args["batch_size"], -1
            )
            preds = network(obs_repeat, repeat_actions)
        ...
        return preds, repeat_actions.view(
            self.num, self.args["batch_size"], -1
        )
    ...
    def _train(self, batch):
        ...
        if self.task in [
            ...
            next_action_diff = torch.sum(
                (new_next_actions - pesudo_next_action) ** 2,
                dim=-1,
                keepdim=True,
            )
        ...
        if self.task in [
            ...
            qf1_deviation = q1_ood_pred - (
                pesudo_q_target - 5.0
            )  # minus delta
            qf1_deviation[qf1_deviation <= 0] = 0
            qf2_deviation = q2_ood_pred - (
                pesudo_q_target - 5.0
            )  # minus delta
            qf2_deviation[qf2_deviation <= 0] = 0

            q1_curr_diff = torch.sum(
                (q1_ood_curr_act - q1_curr_act) ** 2, dim=-1, keepdim=True
            )
            q1_next_diff = torch.sum(
                (q1_ood_next_act - q1_next_act) ** 2, dim=-1, keepdim=True
            )
            q2_curr_diff = torch.sum(
                (q2_ood_curr_act - q2_curr_act) ** 2, dim=-1, keepdim=True
            )
            q2_next_diff = torch.sum(
                (q2_ood_next_act - q2_next_act) ** 2, dim=-1, keepdim=True
            )
            ...
            qf1_loss = (
                self.lam * ((q1_pred - q_target) ** 2).mean()
                + (1 - self.lam) * (q1_weight * qf1_deviation**2).mean()
            )
            qf2_loss = (
                self.lam * ((q2_pred - q_target) ** 2).mean()
                + (1 - self.lam) * (q2_weight * qf2_deviation**2).mean()
            )
        else:
            # stop vanilla pessimistic estimate becoming large
            qf1_deviation = q1_ood_pred - pesudo_q_target
            ...
            qf1_loss = (
                self.lam * self.critic_criterion(q1_pred, q_target)
                + (1 - self.lam) * qf1_ood_loss
            )
            qf2_loss = (
                self.lam * self.critic_criterion(q2_pred, q_target)
