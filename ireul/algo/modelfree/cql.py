...
from ireul.utils.net.common import Net
...
def algo_init(args):
    ...
    if args["obs_shape"] and args["action_shape"]:
        ...
        from ireul.utils.env import get_env_shape
...
class AlgoTrainer(BaseAlgo):
    ...
    def _train(self, batch):
        ...
        if self.args["type_q_backup"] == "max":
            ...
            target_q_values = self.args["q_backup_lmbda"] * torch.min(
                target_q1_next, target_q2_next
            ) + (1 - self.args["q_backup_lmbda"]) * torch.max(
                target_q1_next, target_q2_next
            )
            target_q_values = target_q_values - alpha * new_log_pi
            ...
            target_qf1_values = (
                self._get_tensor_values(
                    next_obs, next_actions_temp, network=self.critic1
                )
                .max(1)[0]
                .view(-1, 1)
            )
            target_qf2_values = (
                self._get_tensor_values(
                    next_obs, next_actions_temp, network=self.critic2
                )
                .max(1)[0]
                .view(-1, 1)
            )
        ...
        if self._current_epoch % 1000 == 0:
            logger.info(
                "Pi loss and critic loss: {}, {}, {}".format(
                    policy_loss.item(), qf1_loss.item(), qf2_loss.item()
                )
            )
